...
cyl(R, T, (0, 0, 0), METAL_STEEL, rot=(math.pi / 2, 0, 0), verts=32, bevel=0.06)
# Raised inner plate and a dark rim groove.
cyl(R * 0.92, T + 0.08, (0, 0, 0), METAL_DARK, rot=(math.pi / 2, 0, 0), verts=32, bevel=0.03)
cyl(R * 0.62, T + 0.16, (0, 0, 0), METAL_STEEL, rot=(math.pi / 2, 0, 0), verts=32, bevel=0.03)
# Rivets around the rim.
for i in range(12):
    a = i / 12.0 * math.tau
    sphere(0.07, (math.cos(a) * R * 0.80, -T / 2 - 0.05, math.sin(a) * R * 0.80), METAL_CHROME, segments=8, rings=6)
# Neon progress ring (the game raises its emission as the countdown runs).
torus(R * 0.76, 0.05, (0, -T / 2 - 0.06, 0), NEON_CYAN, rot=(math.pi / 2, 0, 0), major_segments=40, name="ring")
...
for p0, p1 in zip(pts, pts[1:]):
    rod(p0, p1, 0.06, PLASTIC_BLACK, verts=8)
for p in pts[1:-1]:
    sphere(0.065, p, PLASTIC_BLACK, segments=8, rings=6)
